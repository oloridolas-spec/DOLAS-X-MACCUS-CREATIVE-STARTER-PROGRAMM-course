...
const EnrollmentModal: React.FC<EnrollmentModalProps> = ({ isOpen, onClose, course, bankDetails }) => {
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // Here you would typically send data to Formspree or backend
    // For now, move to payment
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-surface border border-white/20 w-full max-w-md p-6 shadow-2xl overflow-hidden min-h-[400px] flex flex-col justify-center">
            ...
            <form onSubmit={handleFormSubmit} className="space-y-4">
              <div>
                ...
                <input
                  type="text"
                  required
                  className="w-full bg-black/30 border border-gray-700 p-3 text-white focus:border-dolas-red outline-none transition-colors"
                  value={formData.name}
              ...
              <button
                type="submit"
                className="w-full bg-white text-black font-bold py-3 mt-4 hover:bg-gray-200 transition-colors uppercase font-mono"
              >
                Proceed to Payment
              </button>
            </form>
        ...
        {step === EnrollmentStep.PAYMENT && (
          <div className="animate-fade-in">
            <h2 className="text-2xl font-bold mb-1 text-white">Finalize Payment</h2>
            <p className="text-gray-400 text-sm mb-6 font-mono">Step 2/2: Transfer Protocol</p>

            <div className="bg-white/5 p-4 border border-dashed border-gray-600 mb-6">
              <div className="flex justify-between mb-2">
                <span className="text-gray-400 text-sm">Amount:</span>
                <span className="text-dolas-orange font-bold text-lg">₦{course.discountPrice.toLocaleString()}</span>
              </div>
              <div className="flex justify-between mb-2">
                <span className="text-gray-400 text-sm">Bank:</span>
                <span className="text-white font-mono">{bankDetails.bankName}</span>
              </div>
              <div className="flex justify-between mb-2">
                <span className="text-gray-400 text-sm">Account:</span>
                <span className="text-white font-mono text-lg tracking-wider">{bankDetails.accountNumber}</span>
              </div>
            ...
            <p className="text-xs text-center text-gray-500 mb-6">
              Transfer the exact amount and click the button below to verify via WhatsApp.
            </p>

            <button
              onClick={handlePaymentConfirm}
              className="w-full bg-maccus-green text-white font-bold py-3 hover:bg-green-600 transition-colors uppercase flex items-center justify-center gap-2"
            >
